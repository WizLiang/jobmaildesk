...
## 2026-09-20 Windows 维护与版本一致性

- 自动检查只缓存成功结果，缓存仅重建已校验的公开发布字段；重启后按当前版本/通道重验，避免提示丢失和失败冷却 24 小时。定时检查保留已下载待安装状态。
- 重复下载只清理 updates 下严格 UUID 命名的普通 ZIP，拒绝链接目录和文件，保留当前包至新下载成功；旧程序备份不自动删除。
- 版本通过 scripts/version.py 同步 pyproject、模块与锁文件；写入失败尝试回滚，回滚失败保留原始备份。Windows 四段数值版本按 alpha/beta/RC/final 递增，越界拒绝，避免 RC 升正式版数值倒退。
- 构建和 CI 校验三处版本，打包额外核对两个 EXE 的 ProductVersion；设置显示当前版本，主界面保持简洁。维护规则统一 Windows 两资产，正式版仍需 PR 主线与人工验收。

## 2026-09-20 邮箱切换、扫描覆盖与自动过滤复查

- 邮箱表单复用凭据仅限同账号；新邮箱必须提供新授权码。保存设置与扫描互斥，当前生效账号单独显示；本地功能允许不配置邮箱。
- 配置先写入，凭据后写入；凭据失败通过仅含配置的 .config-mail-settings-*.tmp 恢复，临时文件纳入隐私清除。提交成功后先应用运行设置，再清理提示与导出；调度失败立即暂停运行并提示重启，避免已保存却继续用旧设置。
- APScheduler shutdown(wait=False) 不会取消已经开始的旧回调。更换设置时先使旧 ScheduledJobs 失效；扫描取得锁后再次检查有效性，再读取凭据。受控线程回归覆盖旧回调在保存之后才获得锁的路径。
- 扫描覆盖独立于稳定邮件 ID，以账号指纹、端口、SSL 和文件夹的哈希分区。首次尝试先持久化所需起点，完整扫描才推进至扫描开始时刻；部分失败、取消和窄范围补扫不得掩盖缺口。旧状态升级后保守补扫 30 天。
- 自动过滤保留最小脱敏标题和定位元数据，不保存正文、发件地址或私密链接。filtered 与人工 ignored 分开列出；恢复校验整数 revision 和全部归属标记，先拒绝索引人工终态，再更新事实。manual_restore 保证重放不撤销人工选择。
- 扫描规则附加 filtered-review-v1，使最近窗口内旧 filtered 索引补齐复查记录；不是公司/岗位解析规则变更。主界面通过持久扫描计数保留部分失败提示。
